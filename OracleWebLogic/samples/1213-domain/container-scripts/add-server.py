...
def randomName():
  return ''.join(
      [random.choice(string.ascii_letters + string.digits) for _ in xrange(6)])
...
msname = os.environ.get('MS_NAME', f'ManagedServer-{randomName()}@{hostname}')
...
cd(f'/Servers/{msname}')
cmo.setMachine(getMBean(f'/Machines/{nmname}'))
cmo.setCluster(getMBean(f'/Clusters/{cluster_name}'))
...
# ----------------------------------
cd(f'/Servers/{msname}/SSL/{msname}')
...
# -----------------------------------------------------------------------------------------
cd(f'/Servers/{msname}/ServerStart/{msname}')
arguments = f'-Djava.security.egd=file:/dev/./urandom -Dweblogic.Name={msname} -Dweblogic.management.server=http://{admin_host}:{admin_port} {memargs}'
